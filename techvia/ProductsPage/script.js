...
function drawPlaceholder(canvas, category) {
  ...
  if (category === "display")
    ...
    ctx.strokeRect(W * 0.15, H * 0.4, W * 0.7, H * 0.2);
  else if (category === "mouse") {
    ctx.beginPath();
    ctx.ellipse(W / 2, H / 2, W * 0.25, H * 0.35, 0, 0, 2 * Math.PI);
    ctx.stroke();
  } else if (category === "chairs")
    ctx.strokeRect(W * 0.3, H * 0.25, W * 0.4, H * 0.5);
...
function renderProducts(cat = "all", search = "") {
  ...
  list.forEach((p, index) => {
    ...
    card.querySelector(".btn-cart").onclick = (e) => {
      e.stopPropagation();
      addToCart(p.id);
    };

    // View Details button - navigate to ProductDetailsPage
    card.querySelector(".btn-details").onclick = () => {
      window.location.href = `../ProductDetailsPage/product-details.html?id=${p.id}`;
    };
  });
}
...
searchInput.addEventListener("input", () => {
  const activeCat = document.querySelector(".categories button.active").dataset
    .cat;
